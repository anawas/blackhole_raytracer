...
# logging.basicConfig(filename='tracer.log', encoding='utf-8', level=logging.DEBUG)
...
def raytrace_schedule(i, schedule, total_shared, q):  # this is the function running on each thread
    ...
    for chunk in schedule:
        ...
        view[:, 1] = ((-y.astype(float) / RESOLUTION[1] + .5) * RESOLUTION[1]) / RESOLUTION[
            0]  # (inverting y coordinate)
        view[:, 2] = 1.0
        ...
        for it in range(NITER):
            ...
            if DISK_TEXTURE_INT != DT_NONE:
                ...
                mask_crossing = np.logical_xor(oldpoint[:, 1] > 0.,
                                               point[:, 1] > 0.)  # whether it just crossed the horizontal plane
                mask_distance = np.logical_and((pointsqr < DISKOUTERSQR),
                ...
                if (diskmask.any()):
                    ...
                    if DISK_TEXTURE_INT == DT_GRID:
                        ...
                        if DISK_INTENSITY_DO:
                            diskcolor = np.einsum('ij,i->ij', bb.colour(temperature),
                                                  DISK_MULTIPLIER * intensity)  # np.maximum(1.*ones,DISK_MULTIPLIER*intensity))
                        else:
            ...
            if mask_horizon.any():
                ...
                if HORIZON_GRID:
                    ...
                    horizoncolour = np.outer(
                        np.logical_xor(np.mod(phi, 1.04719) < 0.52359, np.mod(theta, 1.04719) < 0.52359),
                        np.array([1., 0., 0.]))
                else:
...
if __name__ == "__main__":
    ...
    for i in range(NTHREADS):
        p = multi.Process(target=raytrace_schedule,
